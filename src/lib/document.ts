...
import type { CvAppearance, CvData, Project, Reference, ReferencePlacement } from '../types'
...
export const hasProjectContent = (project: Project) =>
  Boolean(project.title || project.subtitle || project.description || project.period || project.technologies?.length)
...
export function normalizeCv(parsed: Partial<CvData>, fallback: CvData): CvData {
  ...
  const merged: CvData = {
    ...fallback,
    ...parsed,
    experience: (parsed.experience ?? fallback.experience).map((entry) => ({
      ...entry,
      companyLogo: typeof entry.companyLogo === 'string' ? entry.companyLogo : '',
    })),
    ...
    sectionOrder: insertAfter(parsed.sectionOrder ?? fallback.sectionOrder, 'projects', 'education'),
